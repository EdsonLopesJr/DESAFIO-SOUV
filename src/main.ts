...
import { CalendarRepository } from './Infrastructure/repositories/calendar/calendar.repository.prisma';
...
function main() {
  ...
  const aCalendarRepository = CalendarRepository.create(prisma);

  const createCalendarUsecase = CreateCalendarUsecase.create(aCalendarRepository, aServiceRepository);
  const listCalendarUsecase = ListCalendarUsecase.create(aCalendarRepository);
